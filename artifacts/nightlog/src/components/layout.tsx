...
import { Compass, Home, Mail, User, Sparkles, Flame, Radio, LogOut } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="hidden lg:flex fixed top-0 left-0 bottom-0 w-56 flex-col z-50"
              ...
              <li key={href}>
                <Link href={href}
                  className={`group flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-150 ${
                    active ? "bg-white/8 text-white" : "text-white/38 hover:text-white/75 hover:bg-white/4"
                  }`}>
        ...
        {/* City Pulse strip */}
        <div className="mx-3 mb-3 rounded-xl p-3 space-y-2.5"
          ...
          <div className="px-2 flex items-center gap-2.5">
            <div className="relative shrink-0">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-bold text-white"
                style={{ background: user?.avatarGradient ?? "linear-gradient(135deg,#7c5cfc,#f0365a)" }}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[10px] text-white/30 truncate">{user?.city || "online"}</div>
            </div>
            <button onClick={logout}
              className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-white/20 hover:text-white/55 hover:bg-white/6 transition-all"
              title="Sign out">
